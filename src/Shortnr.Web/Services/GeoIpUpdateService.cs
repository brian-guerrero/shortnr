...
public class GeoIpUpdateService : BackgroundService
{
        ...
        GeoIpService geoIpService, IWebHostEnvironment env)
    {
        ...
        if (_enabled)
        {
            // MaxMind recommends including the account id in the user agent so
            // they can identify the caller when diagnosing issues on their side.
            _httpClient.DefaultRequestHeaders.UserAgent.ParseAdd($"shortnr/1.0 (account_id: {accountId})");
        }
    }
