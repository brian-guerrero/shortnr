...
public class SocialCache : ISocialCache
{
    ...
    public void Set(long socialAccountId, SocialData data)
    {
        ...
        // (a long), not a credential or PII — CodeQL's cs/cleartext-storage-of-
        // sensitive-information heuristic flags it purely because the parameter
        // name contains "account". Logging it is deliberate: it's the correlation
        // key needed to troubleshoot cache activity for a specific account.
        // Dismissed as a false positive: code-scanning alert #23.
    ...
    public void Invalidate(long socialAccountId)
    {
        ...
        // See the justification on Set() above — same false positive (alert #24).
    ...
    private async Task<SocialData?> LoadFromDatabaseAsync(long socialAccountId, CancellationToken ct)
    {
        ...
        {
            // See the justification on Set() above — same false positive (alert #25).
